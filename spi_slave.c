...
void doDisplay(uint8_t t, char units)
{
    ...
    lcd_string(buf1);
    lcd_set_cursor(1, 0);
    snprintf(buf2, 18, "R:%d G:%d Y:%d IR:%d", gpio_get(LED_R), gpio_get(LED_G), gpio_get(LED_Y));
    lcd_string(buf2);
    printf("OLED:\n %s\n%s\n", buf1, buf2);
}
...
static inline uint8_t pack_led_states(bool red, bool yellow, bool green)
{
    return (green << 0) | (yellow << 1) | (red << 2);
}
...
#warning i2c/lcd_1602_i2c example requires a board with I2C pins
    ...
    while (true)
    {
        ...
        out_buf[2] = pack_led_states(red_state, yellow_state, green_state);
        ...
        printf("Core0 F       : %dF\n", F);
        printf("Core0 C       : %dC\n", C);
        printf("%s\n", byte_to_binary(out_buf[2]));
        printf("RECEIVED BUFFER: %02x %02x %02x\n", received_data_buf[2], received_data_buf[1], received_data_buf[0]);
        printf("OLD VAL: %d ... RDB: %d\n", old_val, received_data_buf[2]);
        printf("-----------------------------------------\n");
        */
        ...
        // checking to see if its master haas a bone for it.
        // lcd_clear();
    }
#endif
}